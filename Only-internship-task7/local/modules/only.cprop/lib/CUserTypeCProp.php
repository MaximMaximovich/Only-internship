...
/**
 * Класс добавления комплексного свойства для пользовательских полей
 *
 * Class CUserTypeCProp
 */
class CUserTypeCProp extends CUserTypeString
{
    const USER_TYPE_ID = 'complexCprop';
    ...
    static function GetUserTypeDescription()
    {
        return array(
          ...
          "DESCRIPTION" => Loc::getMessage("ONLY_CPROP_DESC"),
    ...
    {
        $hideText = Loc::getMessage('ONLY_CPROP_HIDE_TEXT');
        $clearText = Loc::getMessage('ONLY_CPROP_CLEAR_TEXT');
        ...
        if (!empty($arUserField['SETTINGS'])) {
            ...
            return '<span>' . Loc::getMessage('ONLY_CPROP_ERROR_INCORRECT_SETTINGS') . '</span>';
        }
        ...
        foreach ($arFields as $code => $arItem) {
            if ($arItem['TYPE'] === 'string') {
                ...
                $result .= self::showFile($code, $arItem['TITLE'], $arUserField);
    ...
    {
        $btnAdd = Loc::getMessage('ONLY_CPROP_SETTING_BTN_ADD');
        $settingsTitle = Loc::getMessage('ONLY_CPROP_SETTINGS_TITLE');

        self::showJsForSetting($htmlControl["NAME"]);
        ...
        $result = '<tr><td colspan="2" align="center">
            <table id="many-fields-table" class="many-fields-table internal">        
                <tr valign="top" class="heading mf-setting-title">
                   <td>XML_ID</td>
                   <td>' . Loc::getMessage('ONLY_CPROP_SETTING_FIELD_TITLE') . '</td>
                   <td>' . Loc::getMessage('ONLY_CPROP_SETTING_FIELD_SORT') . '</td>
                   <td>' . Loc::getMessage('ONLY_CPROP_SETTING_FIELD_TYPE') . '</td>
                </tr>';
            ...
            foreach ($arSetting as $code => $arItem) {
                $result .= '
                       <tr valign="top">
                           ...
                           <td><input type="text" class="inp-title" size="35" name="' . $htmlControl["NAME"] . '[' . $code . '_TITLE]" value="' . $arItem['TITLE'] . '"></td>
                           <td><input type="text" class="inp-sort" size="5" name="' . $htmlControl["NAME"] . '[' . $code . '_SORT]" value="' . $arItem['SORT'] . '"></td>
                           <td>
                                <select class="inp-type" name="' . $htmlControl["NAME"] . '[' . $code . '_TYPE]">
    ...
    {
        $arFields = self::prepareSetting(unserialize($arProperty['SETTINGS']));
    ...
    private static function showString($code, $title, $arUserField)
    {
        $result = '';

        $v = !empty($arUserField['VALUE'][$code]) ? htmlspecialcharsEx($arUserField['VALUE'][$code]) : '';
        $result .= '<tr>
                    <td align="right">' . $title . ': </td>
                    <td><input type="text" value="' . $v . '" name="' . $arUserField['FIELD_NAME'] . '[' . $code . ']"/></td>
                </tr>';

        return $result;
    }

    private static function showFile($code, $title, $arUserField)
    {
        $result = '';

        if (!empty($arUserField['VALUE'][$code]) && !is_array($arUserField['VALUE'][$code])) {
            $fileId = $arUserField['VALUE'][$code];
        } else if (!empty($arUserField['VALUE'][$code]['OLD'])) {
            $fileId = $arUserField['VALUE'][$code]['OLD'];
        } else {
            $fileId = '';
        }
        ...
        if (!empty($fileId)) {
                        ...
                        <td>
                            <table class="mf-img-table">
                                <tr>
                                    <td>' . $content . '<br>
                                        <div>
                                            <label><input name="' . $arUserField['FILE_NAME'] . '[' . $code . '][DEL]" value="Y" type="checkbox"> ' . Loc::getMessage("ONLY_CPROP_FILE_DELETE") . '</label>
                                            <input name="' . $arUserField['FILE_NAME'] . '[' . $code . '][OLD]" value="' . $fileId . '" type="hidden">
                                        </div>
                                    </td>
                                </tr>
                            </table>                      
            ...
            $result .= '<tr>
                    <td align="right">' . $title . ': </td>
                    <td><input type="file" value="" name="' . $arUserField['FILE_NAME'] . '[' . $code . ']"/></td>
                </tr>';
        }
    ...
    public static function showTextarea($code, $title, $arUserField)
    {
        $result = '';
        ...
        $result .= '<tr>
                    <td align="right" valign="top">' . $title . ': </td>
                    <td><textarea rows="8" name="' . $arUserField['FIELD_NAME'] . '[' . $code . ']">' . $v . '</textarea></td>
                </tr>';

        return $result;
    }

    public static function showDate($code, $title, $arUserField)
    {
        $result = '';
                                ...
                                <tr>
                                    <td style="padding: 0;">
                                        <div class="adm-input-wrap adm-input-wrap-calendar">
                                            <input class="adm-input adm-input-calendar" type="text" name="' . $arUserField['FIELD_NAME'] . '[' . $code . ']" size="23" value="' . $v . '">
                                            <span class="adm-calendar-icon"
                                                  onclick="BX.calendar({node: this, field:\'' . $arUserField['FIELD_NAME'] . '[' . $code . ']\', form: \'\', bTime: true, bHideTime: false});"></span>
                                        </div>
                                    </td>
                                </tr>
                            </table>
    ...
    public static function showBindElement($code, $title, $arUserField)
    {
        $result = '';
        ...
        $result .= '<tr>
                    <td align="right">' . $title . ': </td>
                    <td>
                        <input name="' . $arUserField['FIELD_NAME'] . '[' . $code . ']" id="' . $arUserField['FIELD_NAME'] . '[' . $code . ']" value="' . $v . '" size="8" type="text" class="mf-inp-bind-elem">
                        <input type="button" value="..." onClick="jsUtils.OpenWindow(\'/bitrix/admin/iblock_element_search.php?lang=ru&IBLOCK_ID=0&n=' . $arUserField['FIELD_NAME'] . '&k=' . $code . '\', 900, 700);">&nbsp;
                        <span>' . $elUrl . '</span>
                    </td>
                </tr>';
    ...
    public static function showHtmlEditor($code, $title, $arUserField)
    {
        $result = '';
        $v = !empty($arUserField['VALUE'][$code]) ? htmlspecialcharsEx($arUserField['VALUE'][$code]) : '';
        $arUserField["FIELD_NAME"] = htmlspecialcharsEx($arUserField["FIELD_NAME"]);
        $valueName = $arUserField["FIELD_NAME"] . '[' . $code . ']';

        if (Loader::includeModule("fileman")) {
            ob_start();
            $text_name = preg_replace("/([^a-z0-9])/is", "_", $arUserField["FIELD_NAME"] . "[" . $code . "]");
            $text_type = preg_replace("/([^a-z0-9])/is", "_", $arUserField["FIELD_NAME"] . "[TYPE]");
            ...
            $result = '<tr>
                <td align="right" valign="top">' . $title . ': </td>
                <td>' . preg_replace("/name=\".*?\"/", "name=\"$valueName\"", ob_get_contents()) . '</td>
            </tr>';
        }
        ob_end_clean();
        return $result;
    ...
    {
        $showText = Loc::getMessage('ONLY_CPROP_SHOW_TEXT');
        $hideText = Loc::getMessage('ONLY_CPROP_HIDE_TEXT');
    ...
    private static function getOptionList($selected = 'string')
    {
        $result = '';
        $arOption = [
          'string' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_STRING'),
          'file' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_FILE'),
          'text' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_TEXT'),
          'date' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_DATE'),
          'element' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_ELEMENT'),
          'html_editor' => Loc::getMessage('ONLY_CPROP_FIELD_TYPE_HTML_EDITOR')
        ];
